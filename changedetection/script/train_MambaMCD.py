...
import MambaCD.changedetection.utils_func.custom_loss as C
...
class Trainer(object):
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            if itera == 0 and self.args.resume is not None:
    ...
    def validation(self, itera):
        ...
        val_data_loader = DataLoader(dataset, batch_size=int(self.args.batch_size/2), num_workers=6, drop_last=False)
        torch.cuda.empty_cache()
        ...
        with torch.no_grad():
            with tqdm(total=int(np.ceil(len(dataset)/(self.args.batch_size/2))), desc=f"Process on validation...") as pbar:
        ...
        return np.mean(rec), np.mean(pre), np.mean(f1), np.mean(iou), oa, kc
